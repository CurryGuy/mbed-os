...
bool STM32_EMAC::low_level_init_successful()
{
    ...
    for(unsigned idx = 0; idx < ETH_RX_DESC_CNT; ++idx)
    {
...
bool STM32_EMAC::link_out(emac_mem_buf_t *buf)
{
    ...
    success = HAL_ETH_Transmit(&EthHandle, &TxConfig, 0) == HAL_OK;
...
int STM32_EMAC::low_level_input(emac_mem_buf_t **buf)
{
    ...
    if (HAL_ETH_GetRxDataBuffer(&EthHandle, &RxBuff) == HAL_OK)
    {
        ...
        if (frameLength > 0) {
            *buf = memory_manager->alloc_heap(frameLength, 0);
        }

        if (*buf != NULL) {
            memory_manager->copy_to_buf(*buf, RxBuff.buffer, frameLength);
        }
    }
    else
    {
